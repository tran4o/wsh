	...
	io.on('connection', function (socket) {
		...
		socket.on("wsh-connect", function (data, fn) {
			...
			console.error(`ERROR in wsh-connect: sockets[data.code] not defined (${data.code})`);
				...
				if (!c)
					return fn();
				semit(c.socket, "client-disconnect", {
					channel: channel
				});
			...
			channels[channel] = {
				socket: socket,
				forwardTo: sockets[data.code],
				channel: channel,
				code: data.code
			};
			semit(sockets[data.code].socket, "wsh-connect", {
				channel: channel
			});
			return fn();
		});
		socket.on("wsh-disconnect", function (data, fn) {
			try {
				...
				if (!sockets[c.code])
					return;
				semit(sockets[c.code].socket, "wsh-disconnect", {
		...
		socket.on("wsh-data", function (data, fn) {
			try {
				...
				function oneData() {
					...
					if (!sockets[c.code])
						return;
					semit(sockets[c.code].socket, "wsh-data", {
						channel: c.channel,
						data: d
					}, function () {
						if (!s._data) {
